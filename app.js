/* ===========================================================
   ...
   ===========================================================
   Uses MediaPipe Hands to track hand gestures and render
   MULTIPLE procedural glowing flowers (each a different color)
   that bloom, grow, and sway with wind — all on a Canvas
   overlay atop the webcam feed. All flowers share the same
   gesture-driven bloom / growth / wind state, but each has its
   own hue and a slightly offset sway timing so they feel alive
   and independent rather than perfectly synced.
   =========================================================== */
...
class Particle {
    ...
    reset(initial = false) {
        ...
        // Pollen picks up a random hue from a soft floral spread so it
        // reads as coming from "any" of the flowers, not just pink.
...
class FlowerBloomApp {
    constructor() {
        ...
        // Gesture state (smoothed values) — shared across ALL flowers
        this.bloom = 0;
        this.growth = 0;
        this.windForce = 0;
        ...
        // -----------------------------------------------------
        // Multiple flowers, each with its own color + placement.
        // xRatio: horizontal position (0 = left edge, 1 = right edge)
        // hue: base HSL hue for this flower's petals
        // heightMult / scaleMult: relative stem height / flower size,
        //   used to create a bit of depth (some flowers smaller/shorter)
        // phase: unique time offset so sway/flutter isn't synced
        // -----------------------------------------------------
        this.flowerConfigs = [
            { xRatio: 0.12, hue: 345, name: 'Crimson Pink', heightMult: 0.85, scaleMult: 0.85, phase: 0 },
            { xRatio: 0.32, hue: 265, name: 'Violet Purple', heightMult: 1.0, scaleMult: 1.0, phase: 14 },
            { xRatio: 0.52, hue: 30, name: 'Sunset Orange', heightMult: 0.92, scaleMult: 0.95, phase: 27 },
            { xRatio: 0.72, hue: 50, name: 'Golden Yellow', heightMult: 1.05, scaleMult: 1.05, phase: 41 },
            { xRatio: 0.90, hue: 205, name: 'Sky Blue', heightMult: 0.8, scaleMult: 0.8, phase: 58 },
        ];
    ...
    // ----- Stem -----
    // phaseOffset gives each flower its own timing so multiple stems
    // don't sway in perfect unison.
    drawStem(baseX, baseY, height, windAngle, phaseOffset = 0) {
        const ctx = this.ctx;
        const segs = 24;
        const segH = height / segs;
        const localTime = this.time + phaseOffset;
        ...
        for (let i = 1; i <= segs; i++) {
            ...
            const sway = this.noise.get(localTime * 0.6 + i * 0.25, 0) * 10 * t;
            pts.push({
        ...
        // Leaves
        this.drawLeaves(pts, phaseOffset);
    ...
    drawLeaves(stemPts, phaseOffset = 0) {
        const ctx = this.ctx;
        ...
        for (let li = 0; li < positions.length; li++) {
            ...
            const angle = side * (0.45 + this.noise.get(localTime * 0.6 + li * 3, 2) * 0.2);
    ...
    // hue: base color for this flower's petals (each flower passes its own)
    // phaseOffset: unique timing so multiple flowers don't flutter in sync
    drawFlowerHead(cx, cy, bloom, windAngle, scale, hue = 345, phaseOffset = 0) {
        ...
        // --- Ambient glow behind flower (tinted to this flower's hue) ---
        const glowR = (60 + bloom * 120) * adjustedScale;
        if (bloom > 0.02) {
            const glow = ctx.createRadialGradient(0, -glowR * 0.4, 0, 0, -glowR * 0.4, glowR);
            glow.addColorStop(0, `hsla(${hue}, 100%, 65%, ${0.4 * bloom})`);
            glow.addColorStop(0.5, `hsla(${hue}, 100%, 55%, ${0.2 * bloom})`);
        ...
        // Base color derived from this flower's hue
        const sat = 85;
        const light = 55;
        ...
        for (const p of backPetals) {
            const flutter = this.noise.get(localTime * 1.2 + p.angle * 10, 3) * 0.04 * (1 + bloom);
        ...
        if (bloom > 0.15) {
            ...
            for (let i = 0; i < stamenCount; i++) {
                const a = (i / stamenCount) * Math.PI * 2 + localTime * 0.5;
        ...
        for (const p of frontPetals) {
            const flutter = this.noise.get(localTime * 1.4 + p.angle * 10, 4) * 0.03 * (1 + bloom);
    ...
    // ----- Side Branches -----
    drawBranch(startX, startY, baseAngle, length, windAngle, scale, phaseOffset = 0) {
        const ctx = this.ctx;
        const segs = 12;
        ...
        for (let i = 1; i <= segs; i++) {
            ...
            const sway = this.noise.get(localTime * 0.8 + i * 0.3, 5) * 4 * t;
    ...
    // ----- Post-process Glow -----
    // hue: tints the bloom glow to match the flower it surrounds
    drawPostGlow(cx, cy, hue = 345) {
        ...
        g.addColorStop(0, `hsla(${hue}, 100%, 65%, ${this.bloom * 0.18})`);
        g.addColorStop(0.5, `hsla(${hue}, 100%, 55%, ${this.bloom * 0.08})`);
    ...
    animate(timestamp) {
        ...
        // ---- Multiple flowers, each its own color ----
        if (this.growth > 0.005) {
            for (const config of this.flowerConfigs) {
                const stemBaseX = cw * config.xRatio;
                const stemBaseY = ch * 0.95;
                const stemH = ch * 0.45 * this.growth * config.heightMult;
                const flowerScale = 1.25 * this.growth * config.scaleMult;
                const hue = config.hue;
                const phase = config.phase;

                const stemData = this.drawStem(stemBaseX, stemBaseY, stemH, totalWind, phase);
                const tip = stemData.tip;
                const pts = stemData.pts;

                // Draw a couple of side branches and their sub-flowers,
                // in the same hue family as this flower's main bloom.
                const branchConfigs = [
                    { heightRatio: 0.35, direction: -1, lengthFactor: 0.14, scaleFactor: 0.42 },
                    { heightRatio: 0.6, direction: 1, lengthFactor: 0.11, scaleFactor: 0.4 },
                ];

                for (const branch of branchConfigs) {
                    const idx = Math.floor(pts.length * branch.heightRatio);
                    if (idx > 0 && idx < pts.length) {
                        const pt = pts[idx];
                        const prevPt = pts[idx - 1] || pt;
                        const tangent = Math.atan2(pt.y - prevPt.y, pt.x - prevPt.x);

                        const branchAngle = tangent + (branch.direction * 0.75);
                        const branchLength = ch * branch.lengthFactor * this.growth;

                        const branchTip = this.drawBranch(
                            pt.x, pt.y, branchAngle, branchLength, totalWind, this.growth, phase
                        );
                        const subFlowerScale = flowerScale * branch.scaleFactor;

                        this.drawFlowerHead(branchTip.x, branchTip.y, this.bloom, totalWind, subFlowerScale, hue, phase);
                        this.drawPostGlow(branchTip.x, branchTip.y, hue);
                    }
                }

                // Main flower head for this stem
                this.drawFlowerHead(tip.x, tip.y, this.bloom, totalWind, flowerScale, hue, phase);

                // ---- Post-process glow (tinted to this flower's hue) ----
                this.drawPostGlow(tip.x, tip.y, hue);
            }
        }
